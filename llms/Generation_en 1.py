...
Highlight the events that shaped {possessive} legacy and discuss {possessive} impact on society."""
        ...
        self.base_prompt_old = """Write a detailed and vivid paragraph in {language} only about {name} who {present_tense} {fame} {gender} figure {era} who {present_tense} a {role}.
This {gender_noun} {present_tense} {age} and {life_status}. {capital_subject} {existence} and {present_tense} {culture_origin} to the demography of the {language}-language speakers.
Describe {possessive} background, notable actions, and reputation, emphasizing {possessive} role in {timeline}. 
Highlight the events that shaped {possessive} legacy and discuss {possessive} impact on society. Respond in {language} only. If you don't know anything about the entity, make up a story based on the information given."""
...
def inference_vllm_batch(prompt_list, model_name, temperature=0.1, max_tokens=1000):
    ...
    if model_name not in vllm_cache:
        model = LLM(model=model_name, dtype="half", max_model_len=4096)
...
output_csv_file_path = f"{model_n}_{language}_w_names.csv"
